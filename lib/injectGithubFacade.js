...
module.exports = ({
  ...
  /*
  async getPullRequestsSinceLastRelease(ref) {
    const commit = getOneMatching(
  ...
  async getTagSha(tagName) {
    const tag = getOneMatching(
      octokit.repos.listTags,
      { owner, repo },
      t => t.name === tagName
    );
    assert(tag, `Could not find tag matching: ${tagName}`);
    return tag.sha;
  } */
